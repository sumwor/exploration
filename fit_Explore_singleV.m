function [qpar, negloglike, bic, nlike, hess]=fit_Explore_singleV(stats,fit_fun,initpar,x,lb,ub)    
% % fit_fun %
...
inv_TU = 1./ (sqrt(stats.sigmaL.^2 + stats.sigmaR.^2));
...
if ~exist('lb','var')
    ...
    [qpar,negloglike,exitflag,out,grad,hess]=fminunc(func_handle, initpar, op, [c V RU inv_TU]);
else
    [qpar, negloglike, exitflag]=fmincon(func_handle, initpar, [], [], [], [], lb, ub, [], op, [c V RU V_TU]);
end
