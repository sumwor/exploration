function [qpar, negloglike, bic, nlike, hess]=fit_Explore_HMMprior(stats,fit_fun,initpar,x,lb,ub)    
% % fit_fun %
%PURPOSE:   Fit the choice behavior to a model using maximum likelihood estimate
%AUTHORS:   Hongli Wang 2/28/2019
%
%INPUT ARGUMENTS
%   stats:      stats of the game
%               c,r: choice and reward history
%               hmmEst: HMM estimation of states.
...
if ~exist('lb','var')
    ...
    [qpar,negloglike,exitflag,out,grad,hess]=fminunc(func_handle, initpar, op, [c r state],bin);
else
    [qpar,negloglike,exitflag,out,lambda,grad,hess]=fmincon(func_handle, initpar, [], [], [], [], lb, ub, [], op, [c r state],bin);
end
